...
# Case A: Runs Monday but not Sunday -> Train 98045 has MON-SAT
# Monday search
...
print(f"Case A (MON-SAT): Train 98045. Monday (2026-09-21): Found={found_mon}. Sunday (2026-09-20): Found={found_sun}")

# Case D: NOT_SPECIFIED_IN_SOURCE -> Train 98011
...
print(f"Case D (NOT_SPECIFIED): Train 98011. Sunday (2026-09-20): Found={found_d}")

print("\n## D. Direction & Segment Evidence")

# Directionality
res_fwd = client.get('/api/trains/search/?source=CSMT&destination=PNVL')
print(f"Search CSMT -> PNVL returned {len(res_fwd.json())} trains. Numbers: {[t['number'] for t in res_fwd.json()]}")
...
# Segment Slicing
res_seg = client.get('/api/trains/search/?source=CSMT&destination=TNA')
if res_seg.status_code == 200 and len(res_seg.json()) > 0:
    t = res_seg.json()[0]
    print(f"Search CSMT -> TNA Segment extracted:")
    print(f"  Train: {t['number']}")
    print(f"  Source: {t['source']['code']}")
    print(f"  Destination: {t['destination']['code']}")
else:
    print("No train CSMT -> TNA found (possibly not in sample). Checking another...")
...
print(f"Booking with missing distance. Status code: {res_fare.status_code}. Response: {res_fare.json()}")

print("\n## G. Booking Evidence")
# I will do a manual mock booking for a known Express train since suburban is missing distances
res_express = client.get('/api/trains/search/?source=DR&destination=TNA')
if res_express.status_code == 200 and len(res_express.json()) > 0:
    ...
    if b_res.status_code == 201:
        booking = b_res.json()
        print(f"Booking created successfully for {exp_t['number']}.")
        print(f"  Booking From: {booking['source_station']['code']}, To: {booking['destination_station']['code']}")
    else:
        print(f"Booking failed: {b_res.json()}")
else:
    print("Could not find express train to test booking.")
